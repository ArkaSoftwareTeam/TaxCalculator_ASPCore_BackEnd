...
{
    public class UpdateFreeTaxDateTimeCommandHandler : CommandHandler<ChangeFreeTaxDatetime>
    {
        private readonly ITaxCityCommandRepository _taxCityCommandRepository;
        public UpdateFreeTaxDateTimeCommandHandler(ArkaSoftwareCMPOutServices mafiaOuterServices, ITaxCityCommandRepository taxCityCommandRepository) : base(mafiaOuterServices)
        {
            _taxCityCommandRepository = taxCityCommandRepository;
        }

        public override async Task<CommandResult> Handle(ChangeFreeTaxDatetime command)
        {
            ...
            city.ChangeFreeTaxDateTime(new PId(command.FreeTaxDateTimeId) , new FreeTaxDateTime(command.FreeTaxDateTime));
