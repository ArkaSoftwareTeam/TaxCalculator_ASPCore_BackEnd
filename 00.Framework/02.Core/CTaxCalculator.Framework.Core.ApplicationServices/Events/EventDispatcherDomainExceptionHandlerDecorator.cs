...
namespace CTaxCalculator.Framework.Core.ApplicationServices.Events
{
    public class EventDispatcherDomainExceptionHandlerDecorator : EventDispatcherDecorator
    {
        ...
        public override async Task PublishDomainEventAsync<TDomainEvent>(TDomainEvent @event)
        {
            ...
            {
                _logger.LogError(ex, "Processing of {EventType} With value {Event} failed at {StartDateTime} because there are domain exceptions.", @event.GetType(), @event, DateTime.Now);
            }
